...
File inference cache maintenance is centralized in `MeetingLocalInferenceCoordinator`: after native file work returns and before releasing its permit, `MeetingFileInferenceCache` releases unused MLX buffers if they exceed 256 MiB (or when under pressure). Before file admission, the idle lane also reclaims an inherited cache; pressure waits do one pressure-triggered reclamation per episode. This does not unload live weights/state or change global MLX memory limits. The current OS pressure probe is best-effort and preserves notification-based protection if unavailable; a timeout alone never permits inference. Logs distinguish memory pressure, recording and lane occupancy during a wait. The thresholds and pressure-recovery behavior still require macOS validation.
