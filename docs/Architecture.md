...
- History and dictionary persistence use repositories backed by `VoxtDatabase` (GRDB/SQLite), with legacy-data migration. Their domain/Codable values are in `TranscriptionHistoryModels` / `DictionaryModels`; stores still own mutable snapshots, caches and writes. Notes and external sync have separate stores/coordinators.
