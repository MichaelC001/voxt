...
Updated: 2026-09-18. Distinguish **the app's current dependency**, **the published fork**, and **the next app integration target**. A passing library workflow is not an app or model-quality release certification.
...
2. Resolve the **whole app** on macOS and commit `Voxt.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved`. The audio CI graph is reference evidence, not a substitute app lockfile.
...
This coding environment is Linux. Pure Foundation state machines were compiled and exercised with a temporary Swift 6.3.2 toolchain, but neither that isolated harness, static syntax checks nor Python tests can type-check the complete macOS Swift/Metal app. When no local Mac is available, record that limitation and leave verification pending; a passing fork workflow does not waive the app validation gate.
...
The first command explicitly permits dependency re-resolution for this upgrade; subsequent runs use the reviewed lockfile strictly. The script builds Debug/Release, runs XCTest, audits the resolved graph and release binary/link maps, and keeps logs/results under `build/model-stack-validation.*`. It does not push or dispatch Actions. Review skipped tests and separately measure real model quality, latency and memory.
